            ...
            <div key={cycle.id} className="px-5 py-4">
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-500">Cycle #{i + 1}</span>
                  {cycle.status === 'closed'
                ...
                <div className="flex items-center gap-3">
                  {cycle.status === 'closed' && (
                    <span className={`text-sm font-bold ${netGold > 0 ? 'text-green-400' : 'text-slate-400'}`}>
              ...
              <div className="flex items-center gap-3 text-sm">
                <div>
                  <div className="text-xs text-slate-500">{String(cycle.sell_date).split('T')[0]}</div>
                  <div className="text-white font-medium">Sold {cycle.sell_weight}B @ ฿{cycle.sell_price.toLocaleString()}</div>
                  <div className="text-xs text-slate-400">฿{cycle.cash_generated.toLocaleString()} cash</div>
                </div>
                ...
                <div>
                  {cycle.buybacks.length > 0 ? (
                    cycle.buybacks.map(b => (
                      <div key={b.id}>
                        <div className="text-xs text-slate-500">{String(b.buy_date).split('T')[0]}</div>
                        <div className="text-white font-medium">Bought {b.buy_weight}B @ ฿{b.buy_price.toLocaleString()}</div>
                      </div>
                    ))
                  ) : (
                    <div className="text-slate-600 text-sm italic">Waiting for dip...</div>
                  )}
                </div>
              </div>
